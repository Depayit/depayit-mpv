...
# 🛡️ ทบทวนเป้าหมายของ DEPAYIT 

หัวใจของโปรเจ็คคือการสร้าง "เกราะป้องกัน" เพื่อไม่ให้คนไทยตกเป็นเหยื่อจากการฉ้อโกงออนไลน์ โดยใช้โมเดล Agency-Service Soft Escrow ที่ได้ถูกพัฒนามาจาก Escrow ที่ใช้กันเฉพาะอสังหาและธนาคาร ซึ่งสิ่งนี้มีประโยชน์มากสำหรับคนทั่วไปคนปกติไม่สามารถเอื้มถึงได้
ระบบตัวกลาง: เราทำหน้าที่เป็น "ตัวแทน" ถือเงินชั่วคราวเพื่อให้แน่ใจว่า "จ่ายแล้วได้ของ" และ "ส่งของแล้วได้เงิน" 
ความปลอดภัยสูงสุด: ใช้มาตรการ Triple Control (ธนาคาร, บริษัท, 2FA) เพื่อควบคุมบัญชีเงินเข้า-ออก
ความโปร่งใส: ทุกขั้นตอนตรวจสอบได้ผ่าน TXID-PIN โดยไม่ต้องสมัคสมาชิกให้ยุ่งยาก 
...
โครงสร้างหลักที่จะใช้บน **PostgreSQL** 
...
## เจาะลึกรายละเอียดตารางสำคัญ (Key Implementation Detailss
...
**`buyer_id`:** ตั้งเป็น Nullable (ว่างไว้) เพราะตอนผู้ขายสร้าง Link ครั้งแรก อาจจะยังไม่รู้ว่าใครเป็นคนซื้อ จนกว่าผู้ซื้อจะกดลิงก์และกรอกข้อมูล

### 2. `ledger_entries` (สมุดบัญชีแยกประเภท - ความปลอดภัยทางการเงิน)

ส่วนที่ทำให้เราต่างจากแอปฯ ทั่วไป เราจะไม่ใช้แค่ฟิลด์ `balance` แต่เราจะบันทึกการไหลของเงินทุกบาท
...
วิธีนี้ทำให้ตรวจสอบได้ตลอดเวลาว่าเงินในบัญชี Bank ของบริษัท ตรงกับยอดหนี้ที่ต้องจ่ายลูกค้าไหม (Reconciliation) ป้องกันเงินรั่วไหล และตรวจสอบง่ายสำหรับ Auditor
